...
func (h *handlerTransaction) UpdatesTransaction(w http.ResponseWriter, r *http.Request) {
	...
	request := new(transactionsdto.TransactionUpdateRequest) //take pattern data submission
	if err := json.NewDecoder(r.Body).Decode(&request); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		response := dto.ErrorResult{Code: http.StatusBadRequest, Message: err.Error()}
		json.NewEncoder(w).Encode(response)
...
func convertResponseTransactionUpdate(u models.Transaction) transactionsdto.TransactionUpdateResponse {
...
func SendMail(status string, transaction models.Transaction) {
	...
	if status != transaction.Status && (status == "success") {
		...
		var CONFIG_SENDER_NAME = "Dumbflix <engkoskostaman759@gmail.com>"
